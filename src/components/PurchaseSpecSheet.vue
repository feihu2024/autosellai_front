    ...
    <view class="purchase-sheet">
      ...
      <view class="product-head">
        <view class="product-icon">
          <image v-if="currentSku?.sku_image || product.main_image" :src="currentSku?.sku_image || product.main_image"
            class="icon-img" mode="aspectFill" />
      ...
      <view class="spec-section" v-if="skuList.length > 0">
        ...
        <view class="sku-options">
          <view v-for="(sku, index) in skuList" :key="sku.id || index" class="sku-option"
            :class="{ active: selectedIndex === index }" @click="selectSpec(index)"><text>{{ sku.sku_name || `规格 ${index
              + 1}` }}</text></view>
        </view>
...
import { navigator, checkPhoneRequired } from '@/utils'
