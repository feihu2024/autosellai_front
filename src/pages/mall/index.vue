  ...
  <view v-if="isGoldTemplate" class="gold-mall">
    <view v-if="mallBanners.length > 0" class="g-shop-banner">
      <view class="g-banner-track" :style="{ transform: `translateX(-${currentBanner * 100}%)` }">
        <view v-for="(banner, idx) in mallBanners" :key="banner.id || idx" class="g-banner-slide"
      ...
      <view class="g-banner-dots" v-if="mallBanners.length > 1">
        <view v-for="(banner, idx) in mallBanners" :key="idx"
    ...
    <scroll-view scroll-x class="g-shop-categories" :show-scrollbar="false">
      <view v-for="cat in productCategories" :key="cat.value" class="g-cat-btn"
        :class="{ active: activeProductCategory === cat.value }" @click="switchProductCategory(cat.value)"><text>{{
          cat.label }}</text></view>
    </scroll-view>
    ...
    <view class="g-product-grid" v-if="!loading && filteredProducts.length > 0">
      <view class="g-product-card" v-for="product in filteredProducts" :key="product.id">
        <view class="g-product-media" @click="goDetail(product)">
          <image v-if="product.main_image" :src="getImageUrl(product.main_image)" mode="aspectFill" />
        ...
        <view class="g-product-actions">
          <view class="g-product-price" :class="{ 'is-member': getMinMemberPrice(product) }">
            <template v-if="getMinMemberPrice(product)">
              <text class="g-member-label"><text class="g-member-tag">会员</text><text class="g-member-amount">￥{{
                getMinMemberPrice(product) }}</text></text>
  ...
  <view v-else class="mall-page">
    <view v-if="mallBanners.length > 0" class="banner-carousel">
      <view class="banner-track" :style="{ transform: `translateX(-${currentBanner * 100}%)` }">
        <view v-for="(banner, idx) in mallBanners" :key="banner.id || idx" class="banner-slide"
          @click="onBannerClick(banner)">
          <image :src="getImageUrl(banner.image_url)" class="banner-img" mode="aspectFill" />
        </view>
      </view>
      <view class="banner-dots" v-if="mallBanners.length > 1">
        <view v-for="(banner, idx) in mallBanners" :key="idx" :class="['banner-dot', { active: currentBanner === idx }]"
    ...
    <view v-else class="hero">
      ...
      <view class="shop-bag">
        <image class="ui-icon" src="/static/icons/common/shop-white.png" mode="aspectFit" />
      </view>
    </view>
    ...
    <scroll-view scroll-x class="chip-scroll" :show-scrollbar="false">
      <view v-for="cat in productCategories" :key="cat.value" class="chip"
        :class="{ active: activeProductCategory === cat.value }" @click="switchProductCategory(cat.value)"><text>{{
          cat.label }}</text></view>
    </scroll-view>
...
.banner-carousel {
  ...
  overflow: hidden;
  margin-bottom: 16px;
  box-shadow: 0 14px 30px rgba(102, 88, 245, 0.16);
}
...
.hero {
  ...
  margin-bottom: 16px;
  background: linear-gradient(135deg, #5c4ff2, #796af4 55%, #9b92f8);
}
...
.mini-buy-btn {
  ...
  font-size: 11px;
  font-weight: 700;
  box-shadow: 0 6px 14px rgba(102, 88, 245, 0.2);
  display: flex;
  align-items: center;
  justify-content: center;
}
...
.g-shop-categories {
  ...
  border-radius: 14px;
  background: #fff;
  box-shadow: 0 3px 8px rgba(82, 65, 44, 0.06);
}
...
.g-cat-btn.active {
  color: #8d5700;
  background: linear-gradient(90deg, #ffe590, #ffd05b);
  font-weight: 700;
}
...
.g-buy-btn {
  width: 100%;
  height: 36px;
  border: 1px solid rgba(232, 157, 0, 0.18);
  border-radius: 20px;
  color: #3a2500;
  background: linear-gradient(90deg, #ffd65d, #ffb91e);
  box-shadow: 0 4px 10px rgba(255, 176, 0, 0.22);
  font-size: 14px;
  font-weight: 700;
  display: flex;
  align-items: center;
