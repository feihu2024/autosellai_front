  ...
  <view v-if="isGoldTemplate" class="gold-home">
    ...
    <view v-if="homeBanners.length > 0" class="g-banner-carousel">
      ...
      <view class="g-banner-dots" v-if="homeBanners.length > 1">
        <view
          v-for="(banner, idx) in homeBanners"
          :key="idx"
          :class="['g-banner-dot', { active: currentBanner === idx }]"
    ...
    <view class="g-search-box">
      ...
      <input
        ...
        type="text"
        confirm-type="search"
        :placeholder="agentVisible ? '搜索智能体 / 功能' : '搜索商品'"
        @input="onSearch"
      />
    ...
    <template v-if="agentVisible">
      ...
      <view class="g-tool-list" v-if="agents.length">
        <view class="g-tool-card" v-for="agent in agents" :key="agent.id">
          <view
            class="g-favorite"
            :class="{ active: agent.is_favorite }"
            @click.stop="toggleFavorite(agent)"
          >
            <image src="/static/tpl-gold/icon-star.png" mode="aspectFit" />
          </view>
          <image class="g-tool-thumb" :src="agent.cover_url || agent.icon || '/static/icons/common/robot.png'" mode="aspectFill" />
    ...
    <template v-else>
      ...
      <view class="g-tool-list" v-if="products.length">
        ...
        >
          <image class="g-tool-thumb" :src="product.main_image || '/static/icons/common/robot.png'" mode="aspectFill" />
          <view class="g-tool-copy">
            <text class="g-tool-name">{{ product.name }}</text>
            <text class="g-tool-desc">{{ product.subtitle || '点击查看详情' }}</text>
          </view>
          <view class="g-use-btn" @click.stop="goToProduct(product)"><text>¥{{ product.min_price || product.retail_price || 0 }}</text></view>
        </view>
      </view>
  ...
  <view v-else class="home-page">
    ...
    <view class="search-field" :class="{ 'has-value': searchKeyword }">
      ...
      <input
        ...
        type="text"
        confirm-type="search"
        :placeholder="agentVisible ? '搜索智能体' : '搜索商品'"
        @input="onSearch"
      />
    ...
    <view class="quick-grid">
      <view
        ...
        @click="onToolClick(tool)"
      >
        <view class="quick-icon"><image class="ui-icon" :src="tool.icon" mode="aspectFit" /></view>
    ...
    <template v-if="agentVisible">
      ...
      <view class="agent-grid" v-if="agents.length">
        <view class="agent-card" v-for="agent in agents" :key="agent.id">
          <view class="square-media" v-if="agent.cover_url || agent.icon">
            <image :src="agent.cover_url || agent.icon" mode="aspectFill" />
          </view>
      ...
      <view class="ad-strip" @click="goExplore" v-if="agents.length">
        <view class="ad-icon"><image class="ui-icon" src="/static/icons/common/wand-brown.png" mode="aspectFit" /></view>
        <view class="ad-copy">
      ...
      <view class="agent-grid" v-if="products.length">
        ...
        >
          <view class="square-media" v-if="product.main_image">
            <image :src="product.main_image" mode="aspectFill" />
          </view>
          <view class="square-media media-icon" v-else>
            <image class="ui-icon media-emoji" src="/static/icons/common/robot.png" mode="aspectFit" />
          </view>
...
import { getMiniappAgents, getMiniappAgentCategories, favoriteAgent, unfavoriteAgent, getMiniappConfig, getUserProfile, getMiniappMallProducts } from '@/api/miniapp'
...
function goToChat(agent: any) {
  navigator.push(`/m/agent/${agent.id}`)
}
...
.orb-a { width: 150px; height: 150px; right: -42px; top: -50px; }
.orb-b { width: 94px; height: 94px; right: 58px; bottom: -52px; }
...
.search-clear {
  ...
  font-size: 20px; color: #a2abc0; flex-shrink: 0;
}
...
.quick-sub { font-size: 9px; color: #75829c; margin-top: 4px; }
...
.eyebrow {
  display: block; color: #6658f5; font-size: 11px;
  font-weight: 800; letter-spacing: 0.7px; margin-bottom: 5px;
}
.heading-title { font-size: 19px; font-weight: 800; color: #202a42; }
.count-pill { padding: 6px 10px; border-radius: 999px; background: #fff; border: 1px solid #e5eaf4; font-size: 11px; color: #75829c; }
.count-num { color: #6658f5; font-size: 13px; font-weight: 800; }

/* 分类 Chip */
.chip-scroll { white-space: nowrap; padding: 1px 1px 8px; margin-bottom: 8px; }
.chip {
  display: inline-flex; align-items: center; justify-content: center;
  min-width: 66px; height: 36px; padding: 0 15px;
  border-radius: 999px; background: #fff; color: #75829c;
  border: 1px solid #e5eaf4; font-size: 13px; margin-right: 9px;
}
.chip.active {
  background: #6658f5; color: #fff; border-color: #6658f5;
...
.agent-card {
  ...
  overflow: hidden; border: 1px solid rgba(229, 234, 244, 0.92);
  box-shadow: 0 8px 24px rgba(57, 70, 112, 0.07);
  display: flex; flex-direction: column;
}
.square-media {
  width: 100%; height: 0; padding-bottom: 100%;
  overflow: hidden; background: #eef2fb; position: relative;
}
.square-media image {
...
.media-icon .media-emoji { width: 54px; height: 54px; }
.favorite-btn {
...
.favorite-icon-set { position: relative; width: 19px; height: 19px; }
.favorite-icon-set .ui-icon { position: absolute; top: 0; left: 0; width: 19px; height: 19px; }
.icon-active { display: none; }
.favorite-btn.active .icon-normal { display: none; }
.favorite-btn.active .icon-active { display: block; }
.agent-content { padding: 12px 13px 13px; display: flex; flex-direction: column; }
.micro-tag {
  align-self: flex-start; padding: 4px 7px; border-radius: 999px;
  color: #6658f5; background: #efedff; font-size: 10px; font-weight: 700;
}
.agent-name {
  font-size: 15px; line-height: 1.35; margin: 8px 0 5px; color: #202a42;
  font-weight: 700;
  overflow: hidden; text-overflow: ellipsis;
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
}
.agent-desc {
  font-size: 11.5px; line-height: 1.55; color: #75829c;
  overflow: hidden; text-overflow: ellipsis;
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
}
.agent-actions { margin-top: 12px; }
.use-agent-btn {
  width: 100%; height: 38px; border-radius: 13px;
...
.empty-state, .loading-state {
  ...
  flex-direction: column; text-align: center; color: #75829c;
}
...
.product-price { font-size: 16px; font-weight: 800; color: #ff4d4f; }

/* 广告横幅 */
.ad-strip {
  width: 100%; min-height: 60px; margin-top: 16px; padding: 10px 12px;
  border-radius: 18px; background: linear-gradient(90deg, #fff1ae, #ffe58d);
  display: flex; align-items: center; gap: 10px; color: #9a6212;
}
.ad-icon {
...
.ad-icon .ui-icon { width: 19px; height: 19px; }
.ad-copy { flex: 1; display: flex; flex-direction: column; }
.ad-strong { font-size: 13px; font-weight: 700; }
.ad-small { font-size: 10px; opacity: 0.78; margin-top: 2px; }
.ad-tag {
...
/* ===== HomeView 金色模板样式 ===== */
.gold-home { padding: 0 15px 20px; background: #fffaf2; }
.g-banner-carousel {
  position: relative; border-radius: 12px; overflow: hidden;
  margin-bottom: 12px; box-shadow: 0 4px 12px rgba(253, 178, 41, 0.15);
}
.g-banner-track { display: flex; transition: transform 0.4s ease; }
.g-banner-slide { flex: 0 0 100%; width: 100%; height: 170px; }
...
.g-banner-dot { width: 11px; height: 3px; border-radius: 99px; background: #d2d1ce; transition: 0.2s; }
.g-banner-dot.active { background: #ff7b1a; }
.g-hero { border-radius: 12px; overflow: hidden; box-shadow: 0 4px 12px rgba(253, 178, 41, 0.15); margin-bottom: 12px; }
.g-hero-img { width: 100%; }
.g-search-box {
  height: 44px; display: flex; align-items: center; gap: 8px;
  padding: 0 13px; margin-bottom: 12px; border: 1px solid #efede8;
...
.g-search-box input { min-width: 0; flex: 1; font-size: 13px; }
.g-search-clear {
  width: 20px; height: 20px; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  font-size: 14px; color: #a4a5a9; flex-shrink: 0;
}
.g-quick-panel {
  display: flex; gap: 4px; padding: 10px 8px 8px; border-radius: 12px;
  background: rgba(255, 255, 255, 0.96); border: 1px solid rgba(240, 237, 230, 0.9);
  box-shadow: 0 4px 12px rgba(118, 94, 51, 0.1); margin-bottom: 12px;
}
.g-quick-action {
  flex: 1; display: flex; flex-direction: column; align-items: center; gap: 6px;
}
.g-quick-action image { width: 48px; height: 48px; border-radius: 10px; }
.g-quick-action text { font-size: 12px; color: #15171b; }
.g-tabs-row {
  display: flex; align-items: center; height: 38px;
  justify-content: space-around; border-bottom: 1px solid #f5f2ed;
  padding: 0 7px; margin-bottom: 8px; background: #fff;
  border-radius: 12px 12px 0 0; border: 1px solid rgba(240, 237, 230, 0.9);
}
.g-tab { position: relative; min-width: 60px; height: 100%; display: flex; align-items: center; justify-content: center; color: #272a2f; font-size: 13px; }
.g-tab.active { color: #ff7b1a; font-weight: 700; }
.g-tab.active::after {
  content: ""; position: absolute; height: 2px; border-radius: 99px;
  background: linear-gradient(90deg, #ff7b1a, #ffa53d); left: 25%; right: 25%; bottom: 2px;
}
.g-tool-list { display: flex; flex-wrap: wrap; gap: 8px; padding-top: 4px; }
.g-tool-card {
  width: calc(50% - 4px); position: relative; display: flex; flex-direction: column;
  align-items: center; padding: 14px 10px 10px; border: 1px solid #f1eeea;
  border-radius: 14px; background: #fff; box-shadow: 0 3px 7px rgba(82, 65, 44, 0.06);
}
.g-tool-thumb { width: 56px; height: 56px; border-radius: 14px; }
.g-tool-copy { width: 100%; text-align: center; margin-top: 8px; }
.g-tool-name { display: block; margin-bottom: 3px; font-size: 13px; font-weight: 700; color: #15171b; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
.g-tool-desc { display: block; overflow: hidden; color: #7c7f85; font-size: 10px; line-height: 1.35; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
.g-favorite { position: absolute; top: 8px; right: 8px; width: 18px; height: 18px; }
.g-favorite image { width: 18px; height: 18px; }
.g-favorite.active image { filter: sepia(1) saturate(8) hue-rotate(354deg); }
.g-use-btn {
  width: 100%; height: 32px; margin-top: 8px; border-radius: 16px;
  font-size: 11px; color: #15171b; font-weight: 600;
  background: linear-gradient(120deg, #ffca38, #ffb62f);
  box-shadow: 0 3px 6px rgba(255, 179, 0, 0.25);
  display: flex; align-items: center; justify-content: center;
}
.g-empty-state, .g-loading {
  min-height: 160px; display: flex; align-items: center; justify-content: center;
  flex-direction: column; text-align: center; color: #999;
}
.g-empty-title { font-size: 13px; color: #69758e; font-weight: 600; }
